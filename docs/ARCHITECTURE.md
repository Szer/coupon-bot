...
2. **Copilot PR Manager** (`copilot-pr-manager.yml`) — a cron workflow (every 5 min) that auto-closes any PR from non-coding agents by detecting `Custom agent used: project/product` in the PR body, and auto-approves pending workflow runs for legitimate Copilot coding agent PRs
