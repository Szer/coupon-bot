...
- `addflow:toggleapp` — переключить тип купона: «📱 Из приложения» ↔ «🧾 Физический».

### Определение типа купона (App / Physical)

OCR-движок автоматически определяет, является ли купон из приложения Dunnes (app coupon) по маркерам в тексте:
- `"Terms and conditions apply"` (регистронезависимо)
- `"VALUEclub"` (регистронезависимо)
- `"Shop & save voucher"` (регистронезависимо)

Если хотя бы один маркер найден — `isAppCoupon = true`. Пользователь может переключить тип кнопкой на этапе подтверждения.

📱 App-купоны помечаются эмодзи 📱 во всех поверхностях UI: `/list`, `/my`, `/added`, `/take`, уведомления.
...
- При аннулировании:
  ...
  3. Уведомление в группу.
  4. Если купон был взят (`taken`) — отправляется личное сообщение тому, кто его взял: «Купон ID:X был аннулирован владельцем. Он больше недоступен.»
