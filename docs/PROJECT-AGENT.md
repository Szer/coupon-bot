...
1. **Close non-coding agent PRs**: Lists all open PRs authored by the Copilot GitHub App and checks the PR body for `Custom agent used: project` or `Custom agent used: product`. If found **and** the PR title does not contain `[WIP]`, the PR is closed with a comment and the branch deleted. PRs with `[WIP]` in the title are skipped — this indicates the agent session is still active and should not be interrupted.
